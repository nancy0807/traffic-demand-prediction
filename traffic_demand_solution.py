...
Evaluation Metric: max(0, 100 * Rý Score)
"""
...
FEATURES = ['day', 'hour', 'minute', 'time_of_day', 'hour_sin', 'hour_cos', 'time_sin', 'time_cos', 'RoadType_enc', 'NumberofLanes', 'LargeVehicles_enc', 'Landmarks_enc', 'Temperature', 'Weather_enc', 'geohash_enc', 'road_lanes', 'is_rush_hour', 'is_night']
...
# Model Training
model = LGBMRegressor(n_estimators=1500, learning_rate=0.03, num_leaves=128, max_depth=8, min_child_samples=20, feature_fraction=0.8, bagging_fraction=0.8, bagging_freq=5, reg_alpha=0.1, reg_lambda=0.1, random_state=42, n_jobs=-1, verbose=-1)

cv_scores = cross_val_score(model, X, y, cv=5, scoring='r2', n_jobs=-1)
print(f"CV R2 scores: {cv_scores.round^(4^)}")
print(f"Mean CV R2: {cv_scores.mean^(^):.4f}")
print(f"Scaled score: {max^(0, 100 * cv_scores.mean^(^)^):.2f}")
